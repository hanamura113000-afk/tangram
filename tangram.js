...
let memStore={};
const loadLs=()=>{ try{ return JSON.parse(localStorage.getItem(LKEY)||'{}'); }catch(*){ return {...memStore}; } };
const saveLs=o=>{ try{ localStorage.setItem(LKEY, JSON.stringify(o)); }catch(*){ memStore={...o}; } };
...
drawPath(ctx,w);
if(ctx.isPointInPath(x,y)){ ctx.restore(); return [p.id](http://p.id/); }
}
...
const [id=drag.id](http://id=drag.id/); state.pieces=state.pieces.map(p=>p.id===id?{...p,offset:[p.offset[0]+dx,p.offset[1]+dy]}:p);
},{passive:false});
canvas.addEventListener('pointerup',e=>{
if(!drag.active) return; e.preventDefault();
const [id=drag.id](http://id=drag.id/); drag.active=false; drag.id=null; drag.last=[0,0];
const {ACTIVE}=currentSets();
...
const others=state.pieces.filter(p=>[p.id](http://p.id/)!==id).flatMap(p=>transform(p.shape,p.offset,p.angle,p.flipped));
...
const byId=rec.byId;
state.pieces=state.pieces.map(p=> byId[String([p.id](http://p.id/))] ? {...p,
offset:[...byId[String([p.id](http://p.id/))].offset],
angle:byId[String([p.id](http://p.id/))].angle,
flipped:!!byId[String([p.id](http://p.id/))].flipped
} : p);
...
state.pieces.forEach(p=>{ byId[String([p.id](http://p.id/))]={ offset:[...p.offset], angle:p.angle, flipped:!!p.flipped }; });
setLayout(t,byId);
