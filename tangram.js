// Tangram AB + PRACTICE（順序確定版）
// 練習① クリア → 説明 →（開始）→ カウントダウン → 1秒フラッシュ → 練習②開始
document.addEventListener('DOMContentLoaded', () => {
  ...
  const getDeviceCategory=()=>/iPad|Android(?!.*Mobile)|Tablet/i.test(navigator.userAgent||'')?'tablet':(/iPhone|Android.+Mobile|Windows Phone|iPod/i.test(navigator.userAgent||'')?'mobile':'pc');
  const buildRowSingle=d=>({token:SHEETS_TOKEN,participant_id:d.participant_id,pattern:d.pattern,block_index:d.block_index,condition:d.condition,trial_index:d.trial_index,puzzle_title:d.puzzle_title,time_sec:d.time_sec,device_category:getDeviceCategory(),prime_exposure_ms:d.prime_exposure_ms});
  const buildRowPlain =d=>({participant_id:d.participant_id,pattern:d.pattern,block_index:d.block_index,condition:d.condition,trial_index:d.trial_index,puzzle_title:d.puzzle_title,time_sec:d.time_sec,device_category:getDeviceCategory(),prime_exposure_ms:d.prime_exposure_ms});
  async function postToSheets(p){ try{ await fetch(SHEETS_ENDPOINT,{method:'POST',headers:{'Content-Type':'application/json'},mode:'no-cors',body:JSON.stringify(p)}); }catch(_){} }

  // ===== 目標形 =====
  const PUZZLES_A=[{title:"アヒル",target:[[482,394],[588,288],[800,288],[800,182],[906,182],[1012,182],[906,288],[906,394],[800,500],[588,500]]},{title:"凹み",target:[[482,182],[588,182],[588,394],[800,394],[800,182],[906,182],[906,500],[482,500]]},{title:"家",target:[[694,76],[906,288],[800,288],[800,500],[588,500],[588,288],[482,288]]},{title:"コマ",target:[[482,394],[588,288],[641,288],[641,182],[747,182],[747,288],[800,288],[906,394],[694,606]]},{title:"サカナ",target:[[332,356],[544,356],[619,281],[469,281],[544,206],[694,206],[844,356],[694,506],[694,612],[588,612],[694,506],[619,431],[544,506],[394,506],[438,462]]},];
  const PUZZLES_B=[{title:"狐",target:[[270,288],[376,394],[482,394],[694,394],[800,394],[800,288],[906,394],[981,319],[981,469],[906,544],[831,469],[694,606],[694,544],[544,544],[482,606],[482,394],[376,288]]},{title:"猫",target:[[244,288],[350,288],[456,394],[456,244],[756,244],[831,319],[831,169],[906,244],[981,169],[981,319],[906,394],[606,394],[456,394],[350,394]]},{title:"ライオン",target:[[482,288],[588,182],[694,288],[694,394],[906,394],[1012,288],[1012,394],[1056,394],[1131,469],[981,469],[1056,544],[756,544],[694,606],[694,500],[588,394],[588,288]]},{title:"ネッシー",target:[[257,425],[363,319],[469,319],[681,319],[756,244],[831,319],[906,244],[981,319],[906,394],[1012,394],[1012,500],[906,500],[906,544],[756,394],[606,394],[469,531],[469,319],[363,425]]},{title:"魚",target:[[482,288],[694,288],[588,182],[694,182],[906,394],[696,606],[588,606],[694,500],[482,500],[588,394]]},];

  // 練習（三角形）
  const PRACTICE_PUZZLE={ title:'練習（三角形）', target:[[606,288],[756,138],[906,288]], };

  const PIECES=[[[0,0],[300,0],[150,150]],[[0,0],[0,300],[150,150]],[[0,0],[150,150],[0,150]],[[0,0],[150,0],[75,75]],[[150,0],[150,150],[75,75]],[[0,150],[75,225],[150,150],[75,75]],[[0,0],[150,0],[225,75],[75,75]],];
  ...
  const startScreen=$('startScreen'), playerEntry=$('playerEntry'),
        patternAEntry=$('patternAEntry'), patternBEntry=$('patternBEntry'), practiceEntry=$('practiceEntry');
  const practiceIntro=$('practiceIntro'), practicePrimeScreen=$('practicePrimeScreen');
  ...
  const btnReset=$('practiceReset'), btnBack=$('practiceBack');
  ...
  function render(){
    ...
    if(state.selectedId!=null){ const p=ts.find(q=>q.id===state.selectedId); if(p){ ctx.save(); ctx.setLineDash([6,4]); drawPoly(ctx,p.world,null,'#e5e7eb',2); ctx.restore(); } }
    ctx.restore();
  ...
  canvas.addEventListener('pointermove',e=>{
    ...
    state.pieces=state.pieces.map(p=>p.id===drag.id?{...p,offset:[p.offset[0]+dx,p.offset[1]+dy]}:p);
  },{passive:false});
  ...
  const setTitle=()=>{ const {ACTIVE}=currentSets(); titleEl.textContent=ACTIVE[state.puzzleIndex].title; };
  ...
  async function flashThen(cb){
    if(!state.allowFlash){ cb?.(); return; }            // ★ 許可制
    await countdown3();
    const {ACTIVE}=currentSets();
  ...
  if(ADMIN){ saveBtn.addEventListener('click',saveAnswer); window.addEventListener('keydown',e=>{ if(e.altKey&&(e.key==='s'||e.key==='S')) saveAnswer(); }); }
  ...
  function judge(){
    ...
    const px=DILATE_PX, tolT=Math.max(3,px), tolP=Math.max(3,px);
    drawDil(ctxT,tgt,tolT);
    polys.forEach(pl=>drawDil(ctxU,pl,0));
    polys.forEach(pl=>drawDil(ctxUd,pl,tolP));
    ...
    ctxX.drawImage(cU,0,0); ctxX.globalCompositeOperation='destination-out'; ctxX.drawImage(cT,0,0); ctxX.globalCompositeOperation='source-over';
    const outside=alphaCount(ctxX,WORLD_W,WORLD_H);

    ctxX.clearRect(0,0,WORLD_W,WORLD_H);
    ctxX.drawImage(cT,0,0); ctxX.globalCompositeOperation='destination-out'; ctxX.drawImage(cUd,0,0); ctxX.globalCompositeOperation='source-over';
  ...
  function secondHalf(){
    ...
    state.allowFlash=true;                     // ABのフラッシュ許可
  ...
  async function onJudge(){
    ...
    postToSheets(buildRowSingle({
      participant_id:(player.value||'').trim(), pattern:state.pattern, block_index:record.blockIndex,
      condition:record.condition, trial_index:((state.puzzleIndex%5)+1), puzzle_title:record.puzzleTitle,
      time_sec:record.timeSec, prime_exposure_ms:record.primeExposureMs
    }));

    alert(`CLEAR!\n課題: ${record.puzzleTitle}\nタイム: ${record.timeSec}秒`);

    if(state.puzzleIndex===4){ state.step='prime'; primeScreen.classList.remove('hidden'); return; }
    if(state.puzzleIndex<(currentSets().ACTIVE.length-1)){
      ...
      const rows=state.results.map(r=>buildRowPlain({
        participant_id:(player.value||'').trim(), pattern:state.pattern, block_index:r.blockIndex,
        condition:r.condition, trial_index:((r.puzzleIndex-1)%5)+1, puzzle_title:r.puzzleTitle,
        time_sec:r.timeSec, prime_exposure_ms:r.primeExposureMs
      }));
      rows.push(buildRowPlain({ participant_id:(player.value||'').trim(), pattern:state.pattern, block_index:'all', condition:'summary', trial_index:0, puzzle_title:'TOTAL', time_sec:total, prime_exposure_ms:'' }));
  ...
  const isNextPrime=()=>currentSets().FLASH[state.puzzleIndex];
  ...
  window.__startGo=function(){
    ...
    if(practiceEntry?.checked){
      state.mode='practice'; state.practicePhase=1; state.allowFlash=false;
      player.value=name||'';
    ...
    (state.pattern==='B'?patternB:patternA).checked=true;
    ...
    patternA.parentElement.classList.remove('hidden'); patternB.parentElement.classList.remove('hidden');
    firstHalf();
  };
  ...
  window.__practiceIntroGo=function(){
    ...
    btnBack.classList.add('hidden');     // ①は戻るなし
  ...
  // 練習：リセット/戻る
  btnReset?.addEventListener('click',()=>{ if(state.mode!=='practice')return; resetPieces(); resetTimer(); startTimer(); });
  btnBack ?.addEventListener('click',()=>{ if(state.mode!=='practice')return; stop(); ui.classList.add('hidden'); stageWrap.classList.add('hidden'); mobileCtrls.classList.add('hidden'); startScreen.classList.remove('hidden'); titleEl.textContent='—'; timerEl.textContent='00:00'; state.step='home'; state.pieces=[]; state.selectedId=null; state.allowFlash=false; });

  // 初期表示
  ui.classList.add('hidden'); stageWrap.classList.add('hidden'); mobileCtrls.classList.add('hidden');
  timerEl.textContent='00:00';
  titleEl.textContent=(currentSets().ACTIVE[0]?.title||'—');
});
